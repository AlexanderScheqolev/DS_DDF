...
Система предотвращает дублирование обработки на трёх уровнях: (1) при приёме документа — по комбинации file_hash + `sender_id`; (2) при выполнении операций — через idempotency_key и атомарную вставку INSERT ... ON CONFLICT; (3) при отправке — по уникальному ключу (channel_id, recipient, message_hash). Это гарантирует, что каждый документ и каждая операция будут обработаны ровно один раз, даже при повторных запросах или сбоях сети.
